...
Acceptance Criteria:
- [x] Supervisor XPC registers via `SMAppService` with correct plist/entitlements
- [x] Worker executable launches per task, exits after completion, and reconnects with provided endpoint
- [x] App can request, cancel, and reconnect to workers through the supervisor API surface
- [x] Retry/backoff policy defined; capability sandbox checklist enforced
...
- 2025-11-29: Added capability enforcement, SMAppService registration checks, worker launcher improvements, and regression tests.
